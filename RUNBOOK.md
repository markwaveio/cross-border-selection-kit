...
**🅰️ 先把会话做对(能避开大多数 503)**:Amazon 对无痕/无登录态会话最容易回 503。
- chrome-devtools MCP 用它**自己的持久 profile**,接管不了你日常 Chrome——**别试图"接管已有浏览器会话",做不到**(会卡在"profile 被占用/无法接管")。
- 第一次抓前,在 MCP 这个持久 profile 窗口里**登录一次 Amazon**(持久化,以后一直在),用登录态会话抓,503 概率大幅下降。
- 遇"profile 被占用":先 `list_pages` 复用已开的页/关掉占用实例重连,**别新起无痕会话裸抓**。

**⛔ 用对会话仍抓不到时怎么办**(绝不能自己编品硬往下跑):
1. 停下,如实告诉用户"Amazon 实时抓取被拦(已用登录态会话仍未拿到真实候选品)"。
2. 给用户三选一:**(a)** 确认已在 MCP profile 登录 Amazon / 稍后重试(**不要再建议"接管已有浏览器"**);**(b)** 用户直接给想验证的品/关键词;**(c)** 基于公开信息推导候选品——但每个品必须标 `candidate_source: "derived_not_scraped"`,报告顶部红字写明"本轮为推导非实抓,需人工复核"。
...
- **⛔ 被验证码/风控拦截时,绝不直接判失败**,按升级阶梯兜底:① 换用 chrome-devtools MCP 浏览器(用它自己的持久 profile,别试图接管你日常 Chrome)重抓 → ② 仍被拦就**主动引导用户在这个 MCP 浏览器窗口里扫码登录** 1688/淘宝账号,登录后该持久 profile 变可信、且登录态长期保留,再重试 → ③ 都不行(用户无账号/登录仍被风控)才标 `manual_confirmation_needed` + 附搜索直达链接。**别停在"换会话"就放弃,也别因"搜索页理论上不需要登录"就跳过引导扫码。**
